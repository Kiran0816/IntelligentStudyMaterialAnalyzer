...
def flashcard_tool(state: AgentState) -> AgentState:
    """NEW — generates flashcards by calling Claude claude-sonnet-4-6."""
    try:
        kwargs: dict[str, Any] = state.get("tool_kwargs", {})
        ...
        logger.info(f"[tool:flashcard] generated {len(flashcards)} cards")
...
def revision_notes_tool(state: AgentState) -> AgentState:
    """NEW — generates structured revision notes via Claude claude-sonnet-4-6."""
    try:
        ...
        logger.info("[tool:revision_notes] done")
